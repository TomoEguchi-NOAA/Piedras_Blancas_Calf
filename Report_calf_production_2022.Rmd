...
# UMEs
UMEs <- data.frame(Season = c("1999/2000", "2019/2020", 
                              "2020/2021", "2021/2022"),
                   Year = c(1999, 2019, 2020, 2021))
...
stats.1994.2021 <- data.frame(Mean = apply(Estimates_1994_2021, 
  ...
  select(Mean, Median, SD, LCL, UCL, Year, Method)
...
stats.2022 <- data.frame(
  ...
  SD = jm.out.2022$jm$sd$Total.Calves,
...
estimates.PandV1 <- rbind(Estimates %>% select(Year, Mean, SD, LCL, UCL, Method),
                         WayneShort %>% select(Year, Mean, SD, LCL, UCL, Method))
...
p.NvsCalves <- ggplot(MC_Nhats %>% na.omit()) +
  ...
  geom_errorbarh(aes(y = Mean, 
                     ...
                     xmax = UCL.y, 
                     color = factor(Year))) +
  xlab("Abundance") + ylab("Mother calf pair") +
  labs(color = "Year")
