...
p.daily.sightings <- ggplot(data.2022.by.day) +
  ...
  xlab("") + ylab("Daily observed number of mother-calf pairs per effor hour")
...
From `r min(as.Date(data.2022$Date, format = "%Y-%m-%d"))` to `r max(as.Date(data.2022$Date, format = "%Y-%m-%d"))`, `r signif(sum(data.2022$Effort),3)` hours of survey were completed. Daily survey effort ranged from zero to `r signif(max(data.2022.by.day$Daily_Effort), 3)`. A total of `r sum(data.2022$Sightings)` mother-calf pairs of gray whales were counted, with the highest daily count of `r max(data.2022.by.day$Daily_Sightings)` pairs on `r data.2022.by.day[data.2022.by.day$Daily_Sightings == max(data.2022.by.day$Daily_Sightings), "Date"] %>% pull()`.  (`r data.2022.by.day[data.2022.by.day$Daily_Sightings == max(data.2022.by.day$Daily_Sightings), "Sightings_per_Effort"] %>% pull()` per survey effort hour, Figure \@ref(fig:Figure-daily-sightings)). 
...
flextable(Estimates %>% dplyr::relocate(Year) %>% dplyr::select(- Method)) %>%
  set_caption(paste0("Estimated abundance (Mean and Median) and 95% lower (LCL) and upper (UCL) confidence limits of gray whale calves from the visual surveys off the Piedras Blancas Lighthouse Station, CA. "))  %>%
  colformat_double(j = "Year", big.mark = "",
                   digits = 0) %>%
  colformat_double(j = c("Mean", "Median", "LCL", "UCL"), digits = 1) %>%
...
```{r compare-with-Nhats-log, echo=FALSE, include=FALSE, cache=TRUE}
p.NvsCalves.log <- ggplot(log.MC_Nhats %>% na.omit()) +
  ...
  geom_errorbarh(aes(y = log.Mean, 
                     ...
                     xmax = log.UCL.y, 
                     color = factor(Year))) +
  xlab("log(Abundance)") + ylab("log(Calf production)")

```


When the estimated number of calves was compared with the estimated abundance of gray whales from the surveys at Granite Canyon, CA, a linear relationship was found in the log-log scale (Figure \@ref(fig:), Table \@ref(tab:))
