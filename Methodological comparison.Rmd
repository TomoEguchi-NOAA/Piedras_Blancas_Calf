...
In the Weller/Perryman method, the observed number of whales was "corrected" for the estimated sighting probability when there was at least one pair was sighted. When no whale was sighted, however, zero whale was considered to be recorded without error. In reality, however, there was non zero probability of recording zero, when at least one whale was present in the study area. Consequently, there was negative bias in the Weller/Perryman method. 
...
jm.out.v1 <- jm.out.v3 <- jm.out.v4 <- jm.out.v5 <- list()
...
So, pooling data by week does not affect the outcome. Using the Poisson likelihood (v5) with weekly counts results in greater estimates than when the binomial likelihood (v1) used. When daily counts were used, the Poisson likelihood (v3) and the binomial likelihoods (v4) resulted in similar estimates.
