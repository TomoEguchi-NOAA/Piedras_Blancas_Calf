...
for(i in 1:length(FILES)){
  ...
  years[i] <- as.numeric(str_split(FILES[k], " Formatted.csv")[[1]][1])
  ...
  if (!file.exists(paste0("RData/calf_estimates_", years[i], ".rds"))){
    jm <- jags(jags.data,
               inits = NULL,
    ...
    saveRDS(list(jm = jm,
                 ...
                 run.date = Sys.Date()),
            file = paste0("RData/calf_estimates_", years[i], ".rds"))
    
  } else {
    jm.out <- readRDS(paste0("RData/calf_estimates_", years[i], ".rds"))
    jm <- jm.out$jm
  }
}
