...
# a period - regular expression
FILES <- list.files(path = rds.path, 
                    pattern = "calf_estimates_v1_[0-9]{4}\\.")
...
stats.1994.2023 <- data.frame(Mean = lapply(posterior.total.calves, 
                                           FUN = mean) %>% unlist(),
...
data.2023 <- read.csv(paste0(data.path, "2023 Formatted_inshore_v2.csv"))

data.2023 %>% group_by(Date) %>%
  summarise(Daily_Sightings = sum(Sightings),
            Daily_Effort = sum(Effort)) %>%
  mutate(Date = as.Date(Date),
         Sightings_per_Effort = Daily_Sightings/Daily_Effort) -> data.2023.by.day 

p.daily.sightings <- ggplot(data.2023.by.day) +
...
if (save.fig)
  ggsave(filename = "figures/daily_sightings_2023.png",
...
p.Nhats <- ggplot(data = stats.1994.2023) +
...
START FROM HERE - BRINGING IN ABUNDANCE ESTIMATES 2023-06-16
...
Durban_estim <- data.frame(Year = c(2007, 2008, 2010, 2011, 2015, 2016, 2022),
                           Season = c("2006/2007", "2007/2008", "2009/2010",
                                      "2010/2011", "2014/2015", "2015/2016",
                                       "2021/2022", "2022/2023"),
                           Nhat = c(20750, 17820, 21210, 20990, 23530, 26960, 16650, 14525),
                           LCL = c(18860, 16150, 19420, 19230, 21270, 24420, 15170, 13195),
                           UCL = c(23320, 19920, 23250, 22900, 25945, 29830, 18335, 13040),
                           Method = "Durban")
