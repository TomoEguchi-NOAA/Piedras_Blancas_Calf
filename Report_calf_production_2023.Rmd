...
# CIs for the past years. v2 refers to the v2 extraction algorithm (mine)
...
FILES <- list.files(path = rds.path, 
                    pattern = "calf_estimates_v2_Mv1_[0-9]{4}\\.")
...
data. <- read.csv(paste0(data.path, season, " Formatted_inshore_v2.csv"))
...
`r signif(Estimates[Estimates$Year == 2023, "UCL"], 4)`), which was one
of the lowest estimates since the survey started in 1994 but showed an increasing trend over the last few years (Figure \@ref(fig:Figure-Nhats)).
