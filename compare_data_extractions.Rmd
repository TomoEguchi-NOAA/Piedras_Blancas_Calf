...
for (k in 1:length(years)){
  ...
  data.2 <- read_csv(file = paste0(dir.2, Y, " Formatted_v2.csv"),
                     col_types = col.def.2)
...
for (k in 1:length(date.dif.effort)){
  daily.summary.list[[which(years == Y)]]$data.1 %>%
    filter(Date == as.Date(date.dif.effort[k])) -> data.1.dif
  
  daily.summary.list[[which(years == Y)]]$data.2 %>%
    filter(Date == as.Date(date.dif.effort[k])) -> data.2.dif
  
  raw.data <- out.list$data.all %>% filter(Date == as.Date(date.dif.effort[k]))   
}
