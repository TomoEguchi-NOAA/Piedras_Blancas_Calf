...
Effort:
```{r}
THE FOLLOWING FUNCTION DOES NOT WORK AT THE MOMENT BECAUSE SHIFTS ARE DEFINED
DIFFERENTLY BETWEEN V1 AND V3. THE FUNCTION NEEDS TO BE MODIFIED. 2023-06-22
...
```{r}
length(effort.dif.list$raw.data[[2]])
```
...
Visibility changed to 5 at 0900, then turned 4 at 0930. So, it was 2.5 hrs of effort, excluding 0630-0700.

V2 considered 0629 - 0700 as search effort.
...
```{r}
length(effort.dif.list$raw.data[[10]])
```
...
Not sure how V1 computed 2.33 hrs for Shift 2.
...
1600 - 1636 was Visibility 5, so it should be discarded. 1640 - 1900 equals to 140 minutes or 2.33 hrs.
...
V2 cut Shift 2 by 15 minutes. Not sure why. Maybe because the Event 1 at 1000 was not included in the data file?
...
Only 9 mother-calf pairs were observed on 5/3 Shift 4.
...
4. `r sightings.dif.list$date.dif[4]`

Data:
```{r}
sightings.dif.list$raw.data[[4]] 
```
...
The only sighting during the first shift was during an off period due to high visibility code (5). So, it should be excluded. 
