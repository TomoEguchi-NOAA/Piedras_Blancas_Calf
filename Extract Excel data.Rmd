...
# has to have four shifts per day... 
...
all.shifts %>% 
  ...
  mutate(Week = ceiling(difftime(Date, min(all.dates), 
                                 units = "weeks")) %>%
           as.numeric()) %>%
  select(Week, Date, Effort, Sightings) -> raw.all.data

# need to change week = 0 to week = 1...
raw.all.data[raw.all.data$Week == 0, "Week"] <- 1
...
out.file.name.raw <- paste0("data/Raw data/", Year, ".csv")
...
if (!file.exists(out.file.name.raw))
  write_csv(raw.all.data,
            file = out.file.name.raw, 
            quote = "none",
            na = "")
