...
data.all <- data.inshore %>% 
  ...
  filter(!is.na(Shift)) %>%
  arrange(Date, Minutes_since_T0) %>%
  mutate(Date.date = format(Date, "%Y-%m-%d"),
         Date.char = as.character(Date)) %>%
  select(-Date) %>%
  relocate(Date.date)
  
data.shift <- find.effort(data.all, max.shift = 4)
...
START HERE... SOME ISSUES BELOW. 2022-06-23
all.shifts %>% 
  left_join(raw.data, by = c("Date.date", "Shift")) %>%
  mutate(Week = ceiling(difftime(as.Date(Date.char), min(all.dates)), 
                        units = "weeks") %>%
           as.numeric(),
         Date = Date.char) %>%
