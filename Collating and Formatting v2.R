...
# From 2022, no need to use this script. I added all necessary steps into
# Extract Excel data.Rmd. That notebook script will create "Formatted data" that 
# can be used in model fitting and estimations.
...
for(i in 1:length(FILES)){
  ...
  if (!file.exists(paste0("data/Formatted Annual Data/",
                          substr(FILES[i],1,4),
                          " Formatted.csv"))){
    #data <- read.csv(paste0(path, "/", FILES[i]), na.strings="")
    data <- read_csv(paste0(path, "/", FILES[i]), na = "")
    #data <- data %>% fill(Date)
    ...
    w_interm_data <- rbind(wknd_data[W_FROMS[1]:W_TOS[1],],
                           data.frame(Week=unlist(rep(data[W_FROMS[1], "Week"],4)) %>% 
                                        as.vector(),
                                      Date=unlist(rep(data[W_FROMS[1], "Date"],4)) %>% 
                                        as.vector(),
                                      Effort=rep(0,4),
                                      Sightings=rep(0,4)))
    ...
    if(mean(hist(w_interm_data$Week,
      ...
      message(paste0(FILES[i], "did not have 7 days in all weeks"))
      #stop()
    }
  }
